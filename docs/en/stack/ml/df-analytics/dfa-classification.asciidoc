...
{classification-cap} is a {ml} method for predicting the class or category of a 
...
{classification-cap} – just like {regression} – is a supervised {ml} method. It 
means that you need to supply a labeled training data set that has some 
{feature-vars} and a {depvar}. The {classification} algorithm identifies the 
...
learned to classify new data. 
Your training data set should be approximately balanced which means the number
of data points belonging to the various classes should not be widely different, 
