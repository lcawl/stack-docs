...
you must have fields that contain valid vector layers (such as {elastic-maps-service}/#file/world_countries[country codes] or 
{elastic-maps-service}/#file/usa_zip_codes[postal codes]), as defined in the {kibana-ref}/maps-connect-to-ems.html[{ems} ({ems-init})].
...
If you have fields that contain valid vector layers, you can see a choropleth 
map, in which each area is colored based on its document count. For example:
