...
sample web logs data set to detect anomalous behavior in the sum of the `bytes`
...
The map is affected by your swim lane selections. It colors each location to
reflect the number of anomalies in that selected time period. Locations that
have few anomalies are indicated in blue; locations with many anomalies are red.
//TBD: Is it possible to view a legend as is the case in the Data Visualizer?
Thus you can visualize the locations that are generating the most anomalies and 
zoom in--particularly when your vector layers define regions, counties, or
postal codes.
